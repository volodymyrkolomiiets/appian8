...
def post_login():
    ...
    if user:
        if sha256_crypt.verify(str(request.form["password"]), user.password):
            ...
            db.session.commit()
            login_user(user)
            return make_response(jsonify({"message": "Logged in",
...
def post_logout():
    if current_user.is_authenticated:
        logout_user()
        return make_response(jsonify({"message": "You are logged in"}))
    return make_response(jsonify({"message": "You are not logged out"}))
...
def get_username(username):
    ...
    if item is not None:
        ...
        response = jsonify({"message": f"Cannot fined username: {username}"}), 404
